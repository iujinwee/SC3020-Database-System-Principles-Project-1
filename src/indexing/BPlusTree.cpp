...
//    int key_index = 0;
//    int count = leafNode->keys[0].key == bpKey.key ? leafNode->keys[0].count : 1;
//
//    // Find the index to insert the key-address pair
//    // Resulting index have already considered duplicated instances
//    while (key_index < leafNode->size && bpKey.key >= leafNode->keys[key_index].key) {
    ...
    if(cur->is_leaf){
        ...
        while(index < m){
            ...
            if(index >= split_index){
                // transfer to new node
                new_node->keys[new_index] = cur->keys[index];
                new_node->children[new_index] = cur->children[index+1];
